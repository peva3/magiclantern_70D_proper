...
CONFIG_INT( "falsecolor.custom_min", falsecolor_custom_min, 60);
CONFIG_INT( "falsecolor.custom_max", falsecolor_custom_max, 90);
...
{
    if (palette == 6) {
        if (i < falsecolor_custom_min) return 0x09;
        if (i > falsecolor_custom_max) return 0x08;
        int range = falsecolor_custom_max - falsecolor_custom_min;
        if (range <= 0) range = 1;
        int pct = (i - falsecolor_custom_min) * 100 / range;
        if (pct < 25) return 0x06;
        if (pct < 50) return 0x31;
        if (pct < 75) return 0x0F;
        return 0x13;
    }
